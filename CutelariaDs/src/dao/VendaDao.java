...
public class VendaDao{
    ...
    public void novaVenda(Venda v){
        ...
        try (PreparedStatement stm = con.prepareStatement(sql)) {
               stm.setInt(1, v.getCliente().getId_cliente());
               stm.setInt(2, v.getFuncionario().getId_funcionario());
    ...
    public void inserirProduto(Venda v){
        ...
        String sql = "call addprodutoAVenda(?,?)";
         JOptionPane.showMessageDialog(null,v.getProduto().getId_produto());
                JOptionPane.showMessageDialog(null,v.getQuantidade());
        try (PreparedStatement stm = con.prepareStatement(sql)) {
 ...
 public List<Itens_venda> listarTodos(String sql) {
        ...
        if (sql == "null" ){             
        sql = "select * from Itens_venda inner join produto on produto.id_produto = Itens_venda.Id_produto where id_venda =( select max(id_venda) from venda)";
        }
        try (PreparedStatement stm = con.prepareStatement(sql)) {
            ...
            while (resultado.next()) {
                ...
                p.setQuant_prod(resultado.getString("produto.quant_prod"));
